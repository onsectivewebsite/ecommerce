...
export default function SellerThreadPage() {
  ...
  const load = React.useCallback(async () => {
    try { setThread(await api.messaging.thread(threadId)); }
  ...
  return (
    <div className="container py-10 max-w-2xl">
      <div className="flex items-center justify-between mb-4">
        <h1 className="font-display text-2xl tracking-tight">Thread · order #{thread.orderId.slice(-8)}</h1>
        <Link href="/messages" className="ons-btn-ghost text-sm">Back</Link>
      </div>
          ...
          {thread.messages.map((m) => (
            <div
              ...
              className={`rounded-md px-3 py-2 ${
                m.senderKind === 'SELLER' ? 'bg-accent-500/10 ml-12' :
            ...
            >
              ...
              {m.attachments.length > 0 && (
                <div className="mt-2 flex flex-wrap gap-2">
                  {m.attachments.map((a) => (
                    <a key={a.key} href={a.url} target="_blank" rel="noreferrer" className="text-accent-300 text-xs underline">attachment</a>
                  ))}
                </div>
              )}
            </div>
