...
export default function LoginPage() {
  ...
  const [err, setErr] = React.useState<string | null>(
    sp.get('denied') === '1' ? 'Shipping-partner role required.' : null,
  );
  ...
  return (
    <div className="container py-16 max-w-md">
      <Card>
        <CardTitle>Onsective Shipping</CardTitle>
        <CardDescription>For approved logistics partners. SHIPPER role required.</CardDescription>
        <form onSubmit={submit} className="mt-6 flex flex-col gap-4">
          <Input label="Email" name="email" type="email" required defaultValue="shipper@onsective.com" />
          <Input label="Password" name="password" type="password" required defaultValue="OnsectiveShip1!" />
          {err && <p className="text-danger text-sm">{err}</p>}
